...
namespace SimpleMvc.Controllers
{
    ...
    public class UserController : BaseController
    {
        ...
        public async Task<IActionResult> Index(string searchName, string searchEmail, int? page)
        {
            ...
            ViewData["CurrentFilterEmail"] = searchEmail;

            bool filterName = !String.IsNullOrWhiteSpace(searchName);
            bool filterEmail = !String.IsNullOrWhiteSpace(searchEmail);

            var self = await _userManager.GetUserAsync(User);
            var users = await _userManager.Users
                .Where(x => x.Id != self.Id
                    && (!filterName || (filterName && x.FirstName.Contains(searchName) || x.LastName.Contains(searchName)))
                    && (!filterEmail || (filterEmail && x.Email.Contains(searchEmail)))
                ).ToListAsync();
